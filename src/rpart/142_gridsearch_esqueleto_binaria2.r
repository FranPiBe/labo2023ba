...
ArbolEstimarGanancia <- function(semillas, param_basicos) {
  ...
  particionar(dataset, division = c(7, 3), agrupa = "clase_binaria1", seed = semillas)
...
dataset$clase_binaria1 <- ifelse(dataset$clase_ternaria == "BAJA+2", "pos", "neg")
...
for (minbucket in c(100,200,300)){
  for (cp in c(-1,-0.5,0)){
