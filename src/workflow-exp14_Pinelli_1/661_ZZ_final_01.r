...
PARAM <- list()
PARAM$experimento <- "ZZ6610_0924"
PARAM$exp_input <- "HT6510_0924"
...
PARAM$semillas <- c(100153, 100361, 100363, 100391, 100393)
...
# y NO va para Kaggle
future_con_clase <- dfuture_ds[clase_ternaria == "" | is.na(clase_ternaria), .N] == 0
...
for (mes in meses_a_predecir){
  ...
  for (modelo_rank in PARAM$modelos_rank) {
    ...
    for (vsemilla in PARAM$semillas)
    {
      ...
      tb_prediccion <-
        dfuture[, list(numero_de_cliente, mes, clase_ternaria)]
      ...
      fwrite(
        tb_prediccion[, list(numero_de_cliente, mes, prob, clase_ternaria)],
        file = nom_pred,
        sep = "\t"
      )
